...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/19 10:05:18 by tgarriss          #+#    #+#             */
/*   Updated: 2022/12/19 10:17:52 by tgarriss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_map_integrity(t_data *data)
{
	...
	while (y < data->map->height)
	{
		...
		{
			if (ft_isinset(data->map->map[y][x], "0NWSED"))
...
int	validate_map(t_map *map, t_data *data)
{
	...
	while (map->map[++y])
	{
		...
		while (map->map[y][++x])
		{
			if (!ft_isinset(map->map[y][x], "01NWSED"))
				return (EXIT_FAILURE);
