...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/16 14:25:48 by yannickst         #+#    #+#             */
/*   Updated: 2022/12/18 16:28:41 by tgarriss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	do_dda(t_data *data)
{
	while (data->dda->hit == 0)
	{
		if (data->dda->sidedistx < data->dda->sidedisty)
			...
			step_y(data);
		if (data->dda->mapy > data->map->height || data->dda->mapy < 1)
			break ;
...
void	vert_slice(t_data *data, t_img *img, int x)
{
	...
	if (data->dda->drawstart < data->dda->drawend)
	{
