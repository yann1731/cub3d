...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/16 14:24:56 by yannickst         #+#    #+#             */
/*   Updated: 2022/12/16 14:24:59 by yannickst        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (data->dda->side == 0 && data->dda->door == false)
		vert_slice(data, data->env->n_wall, x);
	if (data->dda->side == 1 && data->dda->door == false)
		vert_slice(data, data->env->s_wall, x);
	if (data->dda->side == 2 && data->dda->door == false)
		vert_slice(data, data->env->e_wall, x);
	if (data->dda->side == 3 && data->dda->door == false)
		vert_slice(data, data->env->w_wall, x);
	if (data->dda->door == true)
		vert_slice(data, data->door, x);
}
